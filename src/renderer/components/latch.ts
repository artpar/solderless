...
export function drawLatch(
  ...
  ctx.fillText('AWAIT', Math.round(center.sx), Math.round(center.sy) - 2)

  ctx.font = '9px monospace'
  ctx.fillStyle = COLORS.clockWire
  ctx.fillText('⏳ latch', Math.round(center.sx), Math.round(center.sy) + 10)
