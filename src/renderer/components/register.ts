...
export function drawRegister(
  ...
  ctx.fillText(comp.label.slice(0, 12), Math.round(center.sx), Math.round(center.sy) - 2)

  ctx.font = '9px monospace'
  ctx.fillStyle = COLORS.pinText
  ctx.fillText(comp.operation, Math.round(center.sx), Math.round(center.sy) + 10)
